...
class EzvizV2Client
{
    ...
    public $API_ENDPOINT_SWITCH_SOUND_ALARM = "/sendAlarm";
    ...
    function get_JsonLastError()
    {
        ...
        switch ($lastError) {
            case JSON_ERROR_NONE:
                #log::add('jeezvizv2', 'debug', " - Aucune erreur\r\n";
            break;
            case JSON_ERROR_DEPTH:
                log::add('jeezvizv2', 'debug', " - Profondeur maximale atteinte\r\n");
            break;
            case JSON_ERROR_STATE_MISMATCH:
                log::add('jeezvizv2', 'debug', " - Inadéquation des modes ou underflow\r\n");
            break;
            case JSON_ERROR_CTRL_CHAR:
                log::add('jeezvizv2', 'debug', " - Erreur lors du contrôle des caractères\r\n");
            break;
            case JSON_ERROR_SYNTAX:
                log::add('jeezvizv2', 'debug', " - Erreur de syntaxe ; JSON malformé\r\n");
            break;
            case JSON_ERROR_UTF8:
                log::add('jeezvizv2', 'debug', " - Caractères UTF-8 malformés, probablement une erreur d\'encodage\r\n");
            break;
            default:
                log::add('jeezvizv2', 'debug', " - Erreur inconnue\r\n");
            break;
        }
    }
    ...
    function QueryAPIPost($URL, $postData=null, $timeout=DEFAULT_TIMEOUT, $headers=null)
    {
        try {
            ...
            // Check for errors
            if ($response === false) {
                log::add('jeezvizv2', 'debug', "Echec de la requête");
                log::add('jeezvizv2', 'debug', curl_error($ch));
                log::add('jeezvizv2', 'debug', curl_errno($ch));
                //echo "Une erreur est survenue, vérifiez les logs";
            }
            curl_close($ch);
            // Decode the response
            $response_json = json_decode($response, TRUE);    
            log::add('jeezvizv2', 'debug', $response_json);
    ...
    function __construct($timeout=DEFAULT_TIMEOUT, $cloud=null, $connection=null)
    {
        ...
        $this->account =config::byKey('identifiant', 'jeezvizv2');
        $this->password = config::byKey('motdepasse', 'jeezvizv2');
        log::add('jeezvizv2', 'debug', 'identifiant : '.$this->account);
        //log::add('jeezvizv2', 'debug', 'motdepasse : '.$this->password);        
        $this->_sessionId = config::byKey('sessionId', 'jeezvizv2');
        $this->lastLogin = config::byKey('lastLogin', 'jeezvizv2');
        log::add('jeezvizv2', 'debug', '_sessionId : '.$this->_sessionId);
        log::add('jeezvizv2', 'debug', 'lastLogin : '.$this->lastLogin);
    ...
    {        
        ...
        log::add('jeezvizv2', 'debug', '$this->lastLogin : '.$this->lastLogin );   
        log::add('jeezvizv2', 'debug', '(time() - (60*10)) : '.(time() - (60*10)));   
        if  ($force==true || $this->lastLogin === null || $this->lastLogin < (time() - (60*10)))
        {
            log::add('jeezvizv2', 'debug', 'Token périmé, authentification');        
        }
        else
        {            
            log::add('jeezvizv2', 'debug', 'Token encore valide');   
            return True;     
        }

        log::add('jeezvizv2', 'debug', "Login to Ezviz' API at ".$this->LOGIN_URL);
        # Ezviz API sends md5 of password
        $md5pass = md5(utf8_encode($this->password));
        log::add('jeezvizv2', 'debug', 'md5pass : '.$md5pass);
        ...
        {
            log::add('jeezvizv2', 'debug', "Can not login to API\r\n");
        }
        ...
        {
			log::add('jeezvizv2', 'debug', var_dump($response_json));
            return false;
        }
        ...
        try
        {
            ...
            config::save("sessionId", $sessionId, 'jeezvizv2');
            config::save("lastLogin", time(), 'jeezvizv2');
            log::add('jeezvizv2', 'debug', "Login successfull sessionId =".$sessionId."\r\n");
        }
        catch (Exception $e)
        {          
            log::add('jeezvizv2', 'debug', $e);
    ...
    {
        log::add('jeezvizv2', 'debug', "Get data from pagelist API.\r\n");

        if ($max_retries > $this->MAX_RETRIES)
        {
            log::add('jeezvizv2', 'debug', "Can't gather proper data. Max retries exceeded.");
        }

        if ($filter === null)
        {
            log::add('jeezvizv2', 'debug', "Trying to call get_pagelist without filter");
        }
        ...
        {
            log::add('jeezvizv2', 'debug', "Could not access Ezviz' API: " + $e);
        }
        if (array_key_exists("meta",$response_json))
        { 
            if ($response_json["meta"]["code"] == 401)
            {
                ...
                $this->login(true);
                log::add('jeezvizv2', 'debug', "Got 401, relogging (max retries: ".$max_retries.")");
        ...
        if (!$json_result)
        {
            log::add('jeezvizv2', 'debug', "Impossible to load the devices, here is the returned response: ".$response_json);
        }
    ...
    function switch_alarm($serial, $enable, $max_retries=0)
    {
        ...
        try
        {
            ...
            $headers=array("Content-Type: application/x-www-form-urlencoded", 
                "clientType: 1", 
                "customNo: 1000001");
            $response_json = $this->QueryAPIPost($this->DEVICES_URL.$serial."/0".$this->API_ENDPOINT_SWITCH_SOUND_ALARM, $data, $this->_timeout,$headers);
    ...
    {
        log::add('jeezvizv2', 'debug', "Switch privacy status on ALL devices (batch)\r\n");
    ...
    {
        log::add('jeezvizv2', 'debug', "Enable alarm notifications.\r\n");
        if ($max_retries > $this->MAX_RETRIES)
        {
            log::add('jeezvizv2', 'debug', "Can't gather proper data. Max retries exceeded.");
        }
        ...
        {
            log::add('jeezvizv2', 'debug', "Unproper sensibility (should be within 1 to 6).");
        }
    ...
    {
        log::add('jeezvizv2', 'debug', "Get detection sensibility.\r\n");
        if ($max_retries > $this->MAX_RETRIES)
        {
            log::add('jeezvizv2', 'debug', "Can't gather proper data. Max retries exceeded.");
        }
        ...
        {
            //log::add('jeezvizv2', 'debug', "Could not get detection sensibility : ".var_dump($response_json['resultCode']).")");
            return 'Unknown';
        }
        else
        {
    ...
    {
        log::add('jeezvizv2', 'debug', "Enable alarm sound by API.\r\n");
        if ($max_retries > $this->MAX_RETRIES)
        {
            log::add('jeezvizv2', 'debug', "Can't gather proper data. Max retries exceeded.");
        }
        ...
        {
            log::add('jeezvizv2', 'debug', "Invalid soundType, should be 0,1,2: ".$soundType);
        }
    ...
    function data_report($serial, $enable=1, $max_retries=0)
    {
        try {
            ...
            log::add('jeezvizv2', 'debug', "enable: {".$enable."}, operationType: {".$operationType."}");
    ...
    {
        log::add('jeezvizv2', 'debug', "PTZ Control by API.\r\n");
        if ($max_retries > $this->MAX_RETRIES)
        {
            log::add('jeezvizv2', 'debug', "Can't gather proper data. Max retries exceeded.");            
        }
        ...
        if ($command === null)
        {
            log::add('jeezvizv2', 'debug', "Trying to call ptzControl without command");
        }
        if ($action === null)
        {
            log::add('jeezvizv2', 'debug', "Trying to call ptzControl without action");
        }
